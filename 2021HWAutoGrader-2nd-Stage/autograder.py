...
def grader(testCmd, ioData):
    ...
    with open(ioData) as file:
        ...
        _days, _daysReadable = file.readline().split()
        daysReadable = eval(_daysReadable)
        days = eval(_days)
    ...
    for day_i in range(len(fullInfo)):
        ...
        for mig in day['migration']:
            try:
                ...
                migTot += 1
                if cnt > int(len(VMIDMap) / (100/3)):     # 即假设当前有 n 台存量虚拟机，每天可以迁移的虚拟机总量不得超过 5n/1000 向下取整
